...
class StudentTests(APITestCase):
    ...
    def test_create_Student(self):
        """
        Ensure we can create a new student object inside school
        """
        school_ids = self.create_School(2,20)
        url = '/schools/'+str(school_ids[0])+'/students/'
        data = {'first_name': 'Poompatai', 'last_name': 'Puntitpong','age': 20, 'nationality': 'Thailand'}
        response = self.client.post(url, data, format='json')
        ...
        """Invalid School"""
        url = '/students/aaaa/students/'
        data = {'first_name': 'Poompatai', 'last_name': 'Puntitpong','age': 20, 'nationality': 'Thailand'}
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_create_Student_feed_school(self):
        """
        Ensure we can create a new student object inside school not caring the school id passed in param
        """
        school_ids = self.create_School(2,20)
        url = '/schools/'+str(school_ids[0])+'/students/'
    ...
    def test_create_Student_data_type(self):
        ...
        data = {'first_name':123, 'last_name': 123,'age': 20, 'nationality': 123}
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

        """String age but non convertable"""
        data = {'first_name': 'Poompatai', 'last_name': 'Puntitpong','age': 'AAA', 'nationality': 'Thailand'}
        response = self.client.post(url, data, format='json')
    ...
    def test_create_Student_missing_param(self):
        ...
        """Missing first_name"""
        data = {'last_name': 'Puntitpong','age': 20, 'nationality': 'Thailand'}
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_create_Student_param_validate(self):
        ...
        data = {'first_name': 'A'*21, 'last_name': 'A'*21,'age': 20, 'nationality': 'A'*31}
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        ...
        """-1,0,150,151 age"""
        data = {'first_name': 'Poompatai', 'last_name': 'Puntitpong','age': 0, 'nationality': 'Thailand'}
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

        data = {'first_name': 'Poompatai', 'last_name': 'Puntitpong','age': 150, 'nationality': 'Thailand'}
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

        data = {'first_name': 'Poompatai', 'last_name': 'Puntitpong','age': -1, 'nationality': 'Thailand'}
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        ...
        data = {'first_name': 'Poompatai', 'last_name': 'Puntitpong','age': 151, 'nationality': 'Thailand'}
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_put_Student(self):
        ...
        url = '/schools/'+str(school_ids[0])+'/students/' + id + '/'
        ...
        """Not found since school is changed from previous"""
        response = self.client.put(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_put_student_full_school(self):
        ...
        url = '/schools/'+str(school_ids[1])+'/students/'
        data = {'first_name': 'Poompatai', 'last_name': 'Puntitpong','age': 20, 'nationality': 'Thailand'}
        response = self.client.post(url, data, format='json')
        id = response.data['id']
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

        url = '/schools/'+str(school_ids[1])+'/students/' + id + '/'
        data = {'first_name': 'Poompatai', 'last_name': 'Puntitpong','age': 20, 'nationality': 'Thailand', 'school': school_ids[0]}
        response = self.client.put(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_patch_Student(self):
        ...
        """Not found since school is changed from previous patch"""
        response = self.client.patch(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_delete_Student(self):
        """
        Ensure we can delete student object and not messed up on not found id pr wrong school selection
        """
        school_ids = self.create_School(2,20)
        ids=[]
        url = '/schools/'+str(school_ids[0])+'/students/'
        ...
        url = '/schools/'+str(school_ids[1])+'/students/' + ids[0] + '/'
        response = self.client.delete(url, format='json')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

        for i in range(3):
            url = '/schools/'+str(school_ids[0])+'/students/' + ids[i] + '/'
            response = self.client.delete(url, format='json')
            self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_get_Student(self):
        """
        Ensure we can get student object and not messing not found id or wrong school selection
        """
        school_ids = self.create_School(2,20)
        url = '/schools/'+str(school_ids[0])+'/students/'
        data = {'first_name': 'Poompatai', 'last_name': 'Puntitpong','age': 20, 'nationality': 'Thailand'}
    ...
    def test_get_Student_bulk(self):
        """
        Ensure we can get student objects without mixing schools
        """
        school_ids = self.create_School(2,20)
        url = '/schools/'+str(school_ids[0])+'/students/'
        for i in range(10):
        ...
        self.assertEqual(response.data['results'][0]['school'], school_ids[1])

    def test_get_Student_pagination_with_order(self):
        """
        Ensure we can get student objects all with pagination and order without mixing schools
        """
        school_ids = self.create_School(2,200)
        url = '/schools/'+str(school_ids[0])+'/students/'
        for i in range(104):
            data = {'first_name': 'Poompatai', 'last_name': 'Puntitpong','age': 20, 'nationality': str(random.randint(0,1000))+'Thailand'}
            response = self.client.post(url, data, format='json')
    ...
    def test_get_Student_filter(self):
        """
        Ensure we can filter Student by names,age and nationality without mixing schools
        """
        school_ids = self.create_School(2,200)
        url = '/schools/'+str(school_ids[0])+'/students/'
        data = {'first_name': 'Poompatai', 'last_name': 'Puntitpong','age': 20, 'nationality': 'Thailand'}
        response = self.client.post(url, data, format='json')
        data = {'first_name': 'Robert', 'last_name': 'Puntitpong','age': 25, 'nationality': 'Cambodia'}
        response = self.client.post(url, data, format='json')
        data = {'first_name': 'Alan', 'last_name': 'Manny','age': 20, 'nationality': 'Cambodia'}
        response = self.client.post(url, data, format='json')

        """dummy school 2 data which should not be counted"""
        url = '/schools/'+str(school_ids[1])+'/students/'
        data = {'first_name': 'Poompatai', 'last_name': 'Puntitpong','age': 20, 'nationality': 'Thailand'}
        response = self.client.post(url, data, format='json')
        data = {'first_name': 'Robert', 'last_name': 'Puntitpong','age': 25, 'nationality': 'Cambodia'}
